VISH.VirtualTour = (function(V,$,undefined){
  ...
  var drawMap = function(vt){
    ...
    $(vt.pois).each(function(index,poi){
      ...
      addMarkerToCoordinates(vt,poi.lat,poi.long,poi.id);
    });
  }

  var addMarkerToCoordinates = function(vt,lat,long,poi_id){
    return addMarkerToPosition(vt,new google.maps.LatLng(lat,long),poi_id);
  }
